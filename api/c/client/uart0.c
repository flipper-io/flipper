...
void uart0_put(struct _lf_device *device, uint8_t byte);
...
LF_WEAK int uart0_read(struct _lf_device *device, void *destination, uint32_t length) {
    ...
    lf_assert(lf_free(device, buffer), E_UNIMPLEMENTED, "Failed to free uart0_read memory.");
    return (int)retval;
fail:
    if (buffer) lf_assert(lf_free(device, buffer), E_UNIMPLEMENTED, "Failed to free uart0_read memory.");
    return lf_error;
}

#warning Memory not freed if push or invoke fail.
LF_WEAK int uart0_write(struct _lf_device *device, void *source, uint32_t length) {
    lf_assert(device, E_UNIMPLEMENTED, "invalid device");
    void *buffer = NULL;
    lf_return_t retval;
    ...
    lf_assert(lf_free(device, buffer), E_UNIMPLEMENTED, "Failed to free uart0_write memory.");
    return (int)retval;
fail:
    if (buffer) lf_assert(lf_free(device, buffer), E_UNIMPLEMENTED, "Failed to free uart0_read memory.");
    return lf_error;
...
LF_WEAK uint8_t uart0_get(struct _lf_device *device) {
    lf_return_t retval;
    lf_invoke(device, "uart0", _uart0_get, lf_int8_t, &retval, NULL);
    return (uint8_t)retval;
}

LF_WEAK void uart0_put(struct _lf_device *device, uint8_t byte) {
    lf_return_t retval;
    lf_invoke(lf_get_selected(), "uart0", _uart0_put, lf_void_t, &retval, lf_args(lf_infer(byte)));
}

LF_WEAK int uart0_ready(struct _lf_device *device) {
    lf_return_t retval;
    lf_invoke(device, "uart0", _uart0_ready, lf_int_t, &retval, NULL);
    return (int)retval;
}

LF_WEAK int uart0_reset(struct _lf_device *device) {
    lf_return_t retval;
    lf_invoke(device, "uart0", _uart0_reset, lf_int_t, &retval, NULL);
    return (int)retval;
}

LF_WEAK int uart0_setbaud(struct _lf_device *device, uint32_t baud) {
    lf_return_t retval;
    lf_invoke(device, "uart0", _uart0_setbaud, lf_int_t, &retval, lf_args(lf_infer(baud)));
    return (int)retval;
}

LF_WEAK int uart0_configure(struct _lf_device *device) {
    lf_return_t retval;
    lf_invoke(device, "uart0", _uart0_configure, lf_int_t, &retval, NULL);
    return (int)retval;
}

LF_WEAK void uart0_enable(struct _lf_device *device) {
    lf_return_t retval;
    lf_invoke(device, "uart0", _uart0_enable, lf_void_t, &retval, NULL);
}
