...
LF_WEAK int uart0_read(void* destination, uint32_t length) {
	...
	lf_assert(lf_invoke(device, "uart0", _uart0_read, lf_int_t, &retval, lf_args(lf_ptr(buffer), lf_infer(length))) , E_UNIMPLEMENTED, "Failed to invoke uart0_read.");
...
LF_WEAK int uart0_write(void* source, uint32_t length) {
	...
	lf_assert(lf_invoke(device, "uart0", _uart0_write, lf_int_t, &retval, lf_args(lf_ptr(buffer), lf_infer(length))) , E_UNIMPLEMENTED, "Failed to invoke uart0_write.");
