...
LF_WEAK void* libc_memcpy(void* dst, void* src, size_t n) {
	return (void *)(uintptr_t)lf_invoke(lf_get_current_device(), "libc", _libc_memcpy, lf_int32_t, lf_args(lf_infer(dst), lf_infer(src), lf_infer(n)));
}
...
LF_WEAK void* libc_malloc(uint32_t length) {
	return (void *)(uintptr_t)lf_invoke(lf_get_current_device(), "libc", _libc_malloc, lf_int32_t, lf_args(lf_infer(length)));
}
