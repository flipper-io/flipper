...
LF_WEAK int uart0_pull(void* destination, lf_size_t length) {
	return lf_invoke(lf_get_current_device(), "uart0", _uart0_pull, lf_int32_t, lf_args(lf_infer(destination), lf_infer(length)));
}

LF_WEAK int uart0_push(void* source, lf_size_t length) {
	return lf_invoke(lf_get_current_device(), "uart0", _uart0_push, lf_int32_t, lf_args(lf_infer(source), lf_infer(length)));
}
