...
int uart0_read(void* destination, lf_size_t length);
int uart0_write(void* source, lf_size_t length);
...
LF_WEAK int uart0_read(void* destination, lf_size_t length) {
...
LF_WEAK int uart0_write(void* source, lf_size_t length) {
	void *buf = libc_malloc(length);
	lf_assert(buf, failure, E_UNIMPLEMENTED, "Failed to allocate remote uart0 memory.");
