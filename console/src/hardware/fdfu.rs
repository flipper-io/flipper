...
impl<'a> SamBa<'a> {
    pub fn new(bus: &'a mut Uart0, gpio: &'a mut Gpio) -> SamBa<'a> {
    ...
    fn upload(&mut self, destination: u32, data: &[u8]) -> Result<(), Error> {
        ...
        if word & 0x01 != 0 { return Err(FdfuError::SecurityBit.into()); }
        ...
        for (i, page) in data.chunks(512).enumerate() {
            ...
            for _ in 0..4 {
                ...
                if fsr & 0x01 != 0 { break; }
                if fsr & 0x0E != 0 { return Err(FdfuError::FlashWrite(i).into()); }
            }
        };
        ...
        for i in 0..retries {
            ...
            if i >= retries - 1 { return Err(FdfuError::SecurityBit.into()) }
        }
