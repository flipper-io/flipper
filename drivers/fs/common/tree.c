...
fsp fs_add_leaf_with_key(fsp current, uint16_t key) {
	...
	if (_branch != -1) {
		...
		fsp region = nvm_alloc(sizeof(leaf));
		if (!region) {
			error_raise(E_NO_MEM, ERROR_STRING("Failed to allocate external memory for new filesystem leaf."));
			return 0;
		}
		...
		if (!_leaf) {
			error_raise(E_NO_MEM, ERROR_STRING("Failed to allocate memory to satisfy filesystem operation."));
			return 0;
		}
		...
		nvm_push(&region, sizeof(fsp), _branch);
...
void fs_remove_leaf_with_key(fsp parent, uint16_t key) {
	...
	if (_match -> left && _match -> right) {
		...
		nvm_push(&(_match -> left), sizeof(fsp), _match -> _branch);

		/* Change the branch pointer of the leaf's left child to match its new index in the filesystem tree. */
		nvm_push(&(_match -> _branch), sizeof(fsp), forward(_match -> left, leaf, _branch));

		/* Walk the left child of the leaf to find an empty branch pointer to which the leaf's orphaned right child can be appended. */
		uint16_t *key = (uint16_t *) nvm_dereference(forward(_match -> right, leaf, key), sizeof(uint16_t));

		fsp *right = (fsp *) nvm_dereference(forward(_match -> left, leaf, right), sizeof(fsp));
	...
	/* Also not a bad scenario; the leaf we wish to delete only has a right child. */
		...
		nvm_push(&(_match -> right), sizeof(fsp), _match -> _branch);

		/* Change the branch pointer of the leaf's right child to match its new index in the filesystem tree. */
		nvm_push(&(_match -> _branch), sizeof(fsp), forward(_match -> right, leaf, _branch));
	...
	/* Best case scenario, the leaf to be deleted has no children. */
		...
		fsp zero = 0;
		nvm_push(&zero, sizeof(fsp), _match -> _branch);
