...
void usart0_configure(void *baud) {
	...
	host.invoke(_usart, _usart_configure, 1, baud);
	
}

void usart0_enable(void) {
	
	host.invoke(_usart, _usart_enable, NO_ARGS);
	
}

void usart0_disable(void) {
	
	host.invoke(_usart, _usart_disable, NO_ARGS);
	
}

bool usart0_ready(void) {
	
	return (bool)(host.invoke(_usart, _usart_ready, NO_ARGS));
	
}

void usart0_put(uint8_t byte) {
	
	host.invoke(_usart, _usart_put, 1, byte);
	
}

uint8_t usart0_get(void) {
	
	return (uint8_t)(host.invoke(_usart, _usart_get, NO_ARGS));
	
}

void usart0_push(void *source, uint32_t length) {
	
	host.push(_usart, _usart_push, NO_ARGS, source, length);
	
}

void usart0_pull(void *destination, uint32_t length) {
	
	host.pull(_usart, _usart_pull, NO_ARGS, destination, length);
...
void usart1_put(uint8_t byte) {
	...
	host.invoke(_usart1, _usart_put, NO_ARGS);
