...
int lf_network_read(struct _lf_device *device, void *dst, uint32_t length) {
	...
	lf_assert(device, fail, E_NULL, "No endpoint for to device '%s'.", device->name);
	...
	lf_assert(e > 0, fail, E_COMMUNICATION, "Failed to receive data from networked device '%s' at '%s'.", context->host, inet_ntoa(context->device.sin_addr));
	return lf_success;
...
int lf_network_write(struct _lf_device *device, void *src, uint32_t length) {
	...
	lf_assert(e > 0, fail, E_COMMUNICATION, "Failed to send data to networked device '%s' at '%s'.", context->host, inet_ntoa(context->device.sin_addr));
	return lf_success;
...
struct _lf_device *lf_network_device_for_hostname(char *hostname) {
	...
	lf_assert(context->fd > 0, fail, E_SOCKET, "Failed to create socket for network device.");
	struct hostent *host = gethostbyname(hostname);
	lf_assert(host, fail, E_COMMUNICATION, "Failed to find device with hostname '%s' on the network.", hostname);
