...
void *fmr_resolve(struct _fmr_target *target) {
	/* Dereference the pointer to the target module. */
	void *object = (void *)(fmr_module(target -> module));
	/* Dereference a pointer to the recipient function. */
	void *function = ((void **)(object))[target -> function];
	return function;
}
...
void fmr_perform(struct _fmr_packet *packet, struct _fmr_result *result) {
	...
	/* Resolve the address of the target function. */
	void *function = fmr_resolve(&(packet -> target));
	/* Ensure that the function is valid. */
	if (function) {
		/* Perform the function call internally. */
		result -> value = fmr_call(function, packet -> target.argc, (uint16_t)types, packet -> body + dencode_length);
	} else {
