...
nvm_p nvm_alloc(lf_size_t length) {
	block *current, *last, *current_smallest = NULL, *last_smallest = NULL;
	nvm_p _current, _last, _current_smallest = 0, _last_smallest = 0;
	...
	if (available >= length && available >= length + sizeof(uint32_t)) {
		...
		nvm_p block = (_break_value + offsetof(struct _block, next));
