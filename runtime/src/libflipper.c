...
int lf_attach(struct _lf_device *device) {
	lf_assert(device, fail, E_NULL, "Attempt to attach an invalid device.");
...
int lf_detach(struct _lf_device *device) {
	lf_assert(device, fail, E_NULL, "Invalid device provided to detach.");
...
int lf_invoke(struct _lf_device *device, const char *module, lf_function function, lf_type ret, lf_return_t *retval, struct _lf_ll *args) {
	lf_assert(device, fail, E_NULL, "NULL device was provided to '%s'.", __FUNCTION__);
	lf_assert(module, fail, E_NULL, "NULL module was provided to '%s'.", __FUNCTION__);
	...
	lf_assert(m, fail, E_MODULE, "No counterpart found for module '%s'.", module);
	...
	lf_assert(e == lf_success, fail, E_NULL, "Failed to generate a valid call to module '%s'.", module);
	...
	lf_assert(e == lf_success, fail, E_ENDPOINT, "Failed to send message to device '%s'.", device->name);

	e = device->read(device, &result, sizeof(struct _fmr_result));
	lf_assert(e == lf_success, fail, E_ENDPOINT, "Failed to receive message from the device '%s'.", device->name);
	lf_debug_result(&result);
	lf_assert(result.error == E_OK, fail, result.error, "An error occured on the device '%s':", device->name);
	*retval = result.value;
...
int lf_push(struct _lf_device *device, void *dst, void *src, size_t len) {
	lf_assert(device, fail, E_NULL, "NULL device was provided to '%s'.", __FUNCTION__);
	lf_assert(dst, fail, E_NULL, "NULL dst was provided to '%s'.", __FUNCTION__);
	lf_assert(src, fail, E_NULL, "NULL src was provided to '%s'.", __FUNCTION__);
	lf_assert(len, fail, E_NULL, "Zero length provided to '%s'.", __FUNCTION__);
	...
	lf_assert(e == lf_success, fail, E_FMR, "Failed to push data to device '%s'.", device->name);
...
int lf_pull(struct _lf_device *device, void *dst, void *src, size_t len) {
	...
	lf_assert(e == lf_success, fail, E_FMR, "Failed to pull data from device '%s'.", device->name);
...
int lf_dyld(struct _lf_device *device, const char *module, int *idx) {
	lf_assert(device, fail, E_NULL, "NULL device was provided to '%s'.", __FUNCTION__);
	lf_assert(module, fail, E_NULL, "NULL module provided to '%s'.", __FUNCTION__);
	lf_assert(strlen(module) < 16, fail, E_OVERFLOW, "Module name '%s' is invalid. Module names must be 16 characters or less.", module);
