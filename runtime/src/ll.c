...
int lf_ll_append(struct _lf_ll **_ll, void *item, void *deconstructor) {
	lf_assert(_ll, fail, E_NULL, "Invalid list reference provided to '%s'.", __PRETTY_FUNCTION__);
...
void *lf_ll_item(struct _lf_ll *ll, uint32_t index) {
	...
	lf_assert(ll, fail, E_NULL, "End of list reached before item.");
	return ll->item;
fail:
	return NULL;
}
...
int lf_ll_apply_func(struct _lf_ll *ll, lf_ll_applier_func func,  void *_ctx) {
	lf_assert(ll, fail, E_NULL, "Invalid list provided to '%s'.", __PRETTY_FUNCTION__);
	lf_assert(func, fail, E_NULL, "Invalid applier function provided to '%s'.", __PRETTY_FUNCTION__);
	int e = lf_success;
	do {
		e = func(ll->item, _ctx);
