...
int dyld_test(void) {
    ...
    lf_expect_error();
    lf_assert(module == NULL, E_UNIMPLEMENTED, "Module was not NULL.");
    lf_try(lf_module_release(module));
    lf_expect_error();

    lf_try(module = lf_module_create("test", 1));
    lf_expect_success();
    lf_assert(module, E_UNIMPLEMENTED, "Failed to create module.");
    lf_assert(!strcmp("test", module->name), E_UNIMPLEMENTED, "Module name doesn't match.");
    ...
    lf_expect_success();
    lf_assert(device, E_UNIMPLEMENTED, "Device was NULL.");

    lf_assert(device->modules == NULL, E_UNIMPLEMENTED, "Device modules are not NULL.");
    lf_try(dyld_register(NULL, NULL));
    lf_expect_error();
    lf_try(dyld_register(device, module));
    lf_expect_success();
    lf_assert(device->modules, E_UNIMPLEMENTED, "Device modules were NULL.");

    /* Try to get an invalid module. */
    struct _lf_module *loaded = NULL;
    lf_try(loaded = dyld_module(device, "nope"));
    lf_expect_error();
    lf_assert(loaded == NULL, E_UNIMPLEMENTED, "Loaded module was not NULL.");

    lf_try(loaded = dyld_module(device, "test"));
    lf_expect_success();
    lf_assert(loaded == module, E_UNIMPLEMENTED, "Loaded module did not mach.");
