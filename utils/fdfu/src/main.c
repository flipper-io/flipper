...
void sam_ba_jump(struct _lf_device *u2, uint32_t address) {
    char buffer[11];
    sprintf(buffer, "G%08X#", address);
    uart0_write(u2, buffer, sizeof(buffer) - 1);
}

/* Instructs the SAM-BA to write a word to the address provided. */
void sam_ba_write_word(struct _lf_device *u2, uint32_t destination, uint32_t word) {
    char buffer[20];
...
uint8_t sam_ba_read_byte(struct _lf_device *u2, uint32_t source) {
    char buffer[12];
    sprintf(buffer, "o%08X,#", source);
    uart0_write(u2, buffer, sizeof(buffer) - 1);
    uint32_t retries = 0;
    while (!uart0_ready(u2) && retries++ < 8)
        ;
    return uart0_get(u2);
}

/* Instructs the SAM-BA to write a byte from the address provided. */
void sam_ba_write_byte(struct _lf_device *u2, uint32_t destination, uint8_t byte) {
    char buffer[20];
...
uint32_t sam_ba_read_word(struct _lf_device *u2, uint32_t source) {
    char buffer[12];
    ...
    while (!uart0_ready(u2) && retries++ < 8);
    uint32_t result = 0;
    uart0_read(u2, &result, sizeof(uint32_t));
    return result;
}

/* Writes the given command and argument into the EFC0->EEFC_FCR register. */
void sam_ba_write_efc_fcr(struct _lf_device *u2, uint8_t command, uint32_t arg) {
    sam_ba_write_word(u2, REGADDR(EFC0->EEFC_FCR), (EEFC_FCR_FKEY_PASSWD | EEFC_FCR_FARG(arg) | command));
}

/* Moves data from the host to the device's RAM using the SAM-BA and XMODEM protocol. */
int sam_ba_copy(struct _lf_device *u2, uint32_t destination, void *src, uint32_t length) {
    ...
    uart0_write(u2, buffer, sizeof(buffer) - 1);
    lf_assert(uart0_get(u2) == 'C', E_UNIMPLEMENTED, "Failed to get CTS ACK.");
    ...
    while (length) {
        ...
        uart0_write(u2, &packet, sizeof(packet));
    ...
    lf_assert(uart0_get(u2) == ACK, E_UNIMPLEMENTED, "Failed to get EOT ACK.");
...
int enter_normal_mode(struct _lf_device *u2) {
    int tries = 0;
    char ack[2];
...
int main(int argc, char *argv[]) {
    ...
    struct _carbon_device *carbon = carbon_attach();
    struct _lf_device *u2 = carbon_u2(carbon);
    lf_assert(carbon, E_NO_DEVICE, "failed to attach device");
    ...
    lf_assert(enter_normal_mode(u2), E_UNIMPLEMENTED, "Failed to enter normal mode.");
    lf_debug(KGRN " Successfully entered DFU mode." KNRM);

    /* Ensure the security bit is clear. */
    lf_debug("Checking security bit.");
    sam_ba_write_efc_fcr(u2, EEFC_FCR_FCMD_GGPB, 0);
    lf_assert((sam_ba_read_word(u2, REGADDR(EFC0->EEFC_FRR)) & 0x01) == 0, E_UNIMPLEMENTED,
    ...
    lf_assert(sam_ba_copy(u2, _APPLET, applet, sizeof(applet)), E_UNIMPLEMENTED, KRED "Failed to upload copy applet." KNRM);
    lf_debug(KGRN " Successfully uploaded copy applet." KNRM);

    /* Write the stack address into the applet. */
    sam_ba_write_word(u2, _APPLET_STACK, IRAM_ADDR + IRAM_SIZE);
    /* Write the entry address into the applet. */
    sam_ba_write_word(u2, _APPLET_ENTRY, _APPLET + 0x09);
    /* Write the destination of the page data into the applet. */
    sam_ba_write_word(u2, _APPLET_DESTINATION, IFLASH0_ADDR);
    /* Write the source of the page data into the applet. */
    sam_ba_write_word(u2, _APPLET_SOURCE, _PAGEBUFFER);
    ...
    sam_ba_write_efc_fcr(u2, EEFC_FCR_FCMD_SGPB, 0x01);

    printf("Checking GPNVM1 bit.\n");
    sam_ba_write_efc_fcr(u2, EEFC_FCR_FCMD_GGPB, 0);
    uint8_t retries = 0;
    if (!(sam_ba_read_byte(u2, REGADDR(EFC0->EEFC_FRR)) & (1 << 1)) && retries++ < RETRIES) {
        if (retries > RETRIES) {
    ...
    if (argc > 2) {
        if (!strcmp(argv[2], "verify")) {
            ...
            for (uint32_t i = 0; i < total; i++) {
                ...
                uint32_t word = sam_ba_read_word(u2, addr);
