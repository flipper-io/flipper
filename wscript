...
def build(bld):
	...
	for p in [os.path.basename(p) for p in bld.env.PLATFORMS]:
		bld.install_files('${PREFIX}/include/flipper/platforms/' + p, bld.path.ant_glob('platforms/' + p + '/include/**', excl=['**/private/**']))
	# If on Linux, install the udev rule for all Flipper devices.
